...
Let $\boldsymbol d_n = [\boldsymbol h_n \; \boldsymbol x_n]^T$ be a stacked vector of the features transformed by the hidden-layer, and original features. Furthermore, assuming the activation in the output-layer is linear, then the predicted response of the $n$'th observation is: 
\begin{equation}
...
**NB:** Extreme learning machine's (ELM's) are effectively RWNN's without the functional link and can, therefore, be trained using exactly the same set of equations setting $\boldsymbol d_n = \boldsymbol h_n$.
